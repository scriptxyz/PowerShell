function Get-NSNamedConnectionInfo {
...
.NOTES
  ...
  Updated:        2018-07-01
                  Added sorting and filtering to exclude internal services
...
#region Get NS Connection table information
    ...
    If ($LBvServerName)
    {
        # Add vServer name to URI - show ns connectiontable  "CONNECTION.LB_VSERVER.NAME.EQ(\"Web-VServer\")"
        Write-Verbose ("Added the vServer name """ + $LBvServerName + """ to the URI")
        $strArgs = ("?args=filterexpression:" + [System.Web.HttpUtility]::UrlEncode("CONNECTION.LB_VSERVER.NAME.EQ(""" + $LBvServerName + """)"))
    }
        ...
        If ($response.errorcode -eq 0)
        {
            ...
            If ($response.nsconnectiontable)
            {
                ...
                $response.nsconnectiontable | Select-Object @{N='      Source IP'; E={"{0,15}" -F $_.sourceip}}, 
                                                    ...
                                                    @{N='HTTP request'; E={$_.httprequest}} | Sort-Object '      Source IP','Svc type'| Where-Object {$_.'entity name' -ne 'INTERNAL'} | Format-Table -AutoSize
            }
            Else
            {
...
try {
    Get-NSNamedConnectionInfo -NSIP $args[0] -NSUserName $args[1] -NSUserPW $args[2]
}
catch [System.Management.Automation.ParameterBindingException] {
